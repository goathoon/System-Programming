...
int main(int argc, char *argv[])
{
	...
	for(x = 0; x < case_num1; x++) {
		for(y = x; y < case_num2; y++) {
			char *comp_result;
			comp_result = sfp_comp(sf1[x], sf1[y]);
			fscanf(fp_answer, "%s", comp_ans);
			fprintf(fp_output, "%s\n", comp_result);
			printf("%s %s %s, ", sfpbits1[x], comp_result, sfpbits1[y]);
	...
	for(x = 0; x < case_num1; x++) {
		for(y = x; y < case_num2; y++) {
			char *comp_result;
			comp_result = sfp_comp(sf2[x], sf2[y]);
			fscanf(fp_answer, "%s", comp_ans);
			fprintf(fp_output, "%s\n", comp_result);
			printf("%s %s %s, ", sfpbits2[x], comp_result, sfpbits2[y]);
			if(!strcmp(comp_result, comp_ans))
				printf("CORRECT\n");
			else
				printf("WRONG\n");
	...
	for(x = 0; x < case_num1; x++) {
		for(y = 0; y < case_num2; y++) {
			char *comp_result;
			comp_result = sfp_comp(sf1[x], sf2[y]);
			fscanf(fp_answer, "%s", comp_ans);
			fprintf(fp_output, "%s\n", comp_result);
			printf("%s %s %s, ", sfpbits1[x], comp_result, sfpbits2[y]);
			if(!strcmp(comp_result, comp_ans))
				printf("CORRECT\n");
			else
				printf("WRONG\n");
